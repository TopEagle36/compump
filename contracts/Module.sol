...
interface IModule {
    ...
    // function getModuleInfo() external view returns (address, uint8[] memory , uint256[] memory, string memory , string memory , string memory, uint256, address, address[] memory, uint256, uint256);
...
contract Module is OwnableUpgradeable, IModule , ReEntrancyGuard {
    using SafeMath for uint256;
    ...
    function stake(
        ...
        uint256 userTotalStaked = contributionOf[msg.sender].add(_amount);
        if (contributionOf[msg.sender] == 0) {
            IModuleFactory(factory).recordContribution(msg.sender, address(this));
        }
        contributionOf[msg.sender] = userTotalStaked;
    ...
    function unStake(
        ...
        contributionOf[msg.sender].sub(_amount);
        totalRaised = totalRaised.sub(userTotalStaked);
    ...
    function getUserStaked(
        ...
        uint256 userTotalStaked = contributionOf[user];
    ...
    // function getModuleInfo() external override view returns (address, uint8[] memory, uint256[] memory, string memory , string memory , string memory, uint256, address, address[] memory, uint256, uint256){
    ...
    //     return (token, state, info, IERC20(token).name(), IERC20(token).symbol(), moduleDetails, tokenId, v3Pair, _holders, tokenPrice, tokenTotalSupply);
    // }
