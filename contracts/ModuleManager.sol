...
interface IModuleManager {
    ...
    event ModuleForTokenRemoved(address indexed token, address pool);
}
...
contract ModuleManager is OwnableUpgradeable, IModuleManager {
    ...
    function isModuleGenerated(address pool) public view override returns (bool) {
        return _modules.contains(pool);
    }
    ...
    function getUserContributedPoolInfo(
        ...
        EnumerableSet.AddressSet storage poolAddrs = _contributedModulesOf[user];
        for (uint256 i = start; i <= end; i++) {
            (
                address token,
                address currency,
                ...
                uint256 tokenId,
                address v3Pair
            ) = IModule(poolAddrs.at(i)).getModuleInfo();
            lockInfo[currentIndex] = CumulativeLockInfo(
                poolAddrs.at(i),
                token,
                currency,
